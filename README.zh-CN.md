...
| 点击命中区域 | 单面板 / 终端面板（联动关闭或无子面板） | 双面板模式（子面板联动中） | 行为细节 |
| :--- | :--- | :--- | :--- |
| **Chevron 箭头** | 展开 / 折叠目录 | 展开 / 折叠目录 | 始终维持父面板树状展开状态不变。 |
| **文件夹名称 (Name)** | 打开文件夹笔记 (若存在) / 折叠 | 发送焦点并导航子面板 | 若有文件夹笔记 (Folder Note) 则打开笔记（常态显示细下划线标示）。无文件夹笔记时，单面板原地展开/折叠，双面板则导航子面板。 |
| **行背景 / 空白区** | 就地下钻 (Drill-down) | 发送焦点并导航子面板 | 即时将面板根目录切换为该文件夹；双面板时发送焦点给子面板。 |
| **长按 (450ms)** | 就地下钻 (Drill-down) | 就地下钻 (Drill-down) | 在当前面板原地切换根目录，不联动子面板，保留现有布局。 |

### 就地下钻 (In-place Drill-down) 与适用场景

“就地下钻”是 Folder Spaces 在无需打开新标签页或分割窗口的情况下，让当前面板**原地切换根目录、深入子文件夹**的核心导航机制：

- **运作机制**：
  - **即时切换范围**：在终端面板或独立面板中点击文件夹的空白背景（或在任意面板长按 450ms），当前面板的根目录会立即切换至目标文件夹，并自动套用预设的子面板视图预设集（例如自动转为 `Contents` 扁平内容模式，快速总览所有文件）。
  - **历史堆栈与智能返回**：每次下钻都会将上一层路径与视图配置存入历史堆栈（Drill-down Stack）。下钻后，标题栏左侧的状态图标会即时切换为 **“← 返回上一层”按钮**。
  - **一键无损还原**：点击标题栏的 `←` 返回按钮即可退回上一层，并完整还原先前的视图预设集与展开状态；退回顶层后，图标自动恢复为原本的文件夹／联动图标。您也可以直接点击标题栏更换根目录。

#### 典型适用场景

1. **级联终端面板深入探索 (Terminal Panel)**
   - 在 `Parent → Middle → Child` 的多面板级联接龙中，最后的终端面板（如终端 `Files` 或 `Contents` 列表）通常没有再串接下一个面板。
   - 当在终端面板中浏览到包含更深层结构的子文件夹时，**无需再手动多开一个分割面板占用屏幕**。只需点击该文件夹的空白处，终端面板即可就地钻入该子目录；查阅完毕后点击标题栏的 `←` 箭头即可无缝返回。
2. **独立单面板轻量导航 (Single / Standalone Panel)**
   - 在侧边栏或浮动窗口中仅打开单个 Folder Space、未开启多面板联动时，Folder Space 充当轻量且聚焦的项目浏览器。
   - 点击文件夹空白处即可一层层深入子目录，搭配标题栏的 `←` 返回按钮，提供类似 macOS Finder 分栏式下钻或浏览器上一页般的流畅切换体验，既能保持工作区干净，又免除树状目录层层展开后的杂乱视觉负担。
3. **保留布局的临时原地下钻 (Long-Press Drill-down)**
   - 在已联动子面板的父面板中，若想临时深入某个分支文件夹，但**不想切换右侧子面板已锁定的工作上下文**时，直接在该文件夹上“长按 450ms”，即可在父面板就地下钻查看，完全不干扰右侧子面板与现有布局。
