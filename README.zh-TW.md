...
- **檢視預設集**：六個具名預設集，組合檢視、展開層級與內容——`Explorer`（標準樹狀）、`Navigate`（純資料夾樹）、`Columns`（單層資料夾清單）、`Context`（雙層資料夾樹）、`Contents`（扁平總覽）、`Files`（扁平檔案清單）。可套用於每個面板，也可在父面板驅動其子面板鏈時水平套用。
- **過濾與排序**：依路徑子字串過濾，並依名稱、修改時間或建立時間排序（升／降冪）——每個資料夾可獨立設定。
...
- **`檢視設定` 選單**：切換樹狀／扁平、展開層級與內容、預設集、過濾、排序、設定自訂資料夾圖示、顯示／自動顯示目前檔案、全部收合／展開。
- **安全重命名**的資料夾重命名對話框。
- **每個資料夾的設定**（檢視、層級、內容、排序、圖示）會被記住，並在資料夾更名或搬移時自動遷移。

### 多面板接龍連動 (Cascading Panel Binding)
將 Folder Space 面板串成 parent → child → grandchild 的級聯鏈，跨面板帶動資料夾上下文：

- 在任一 Folder Space（或原生檔案總管）中對資料夾按右鍵並開啟新面板——新面板**自動綁定為來源面板的子面板**。
- **「與父面板資料夾焦點連動」** 開關（預設 ON）會讓子面板的資料夾範圍跟隨父面板的資料夾選擇；關閉後子面板範圍保持固定。
...
### 面板不會被取代
- 在 popout 視窗中，側邊欄面板（backlink、outline、outgoing links、search 等）與 Folder Space explorer **不會被新開啟的 note 取代**——note 會開在先前 active 的 note tab 上，或在其 tab group 開新 tab；未 pin 的 note tab 仍維持「開在目前 tab」的一般行為。
...
- **顯示選項**
  - **預設資料夾檢視**：尚未儲存個別模式的資料夾使用的檢視模式（樹狀或扁平）。
  - **預設展開層級**：1 層、2 層或全部層級。
  - **預設內容**：資料夾、檔案或全部。
  - **檢視預設集**：新面板的預設集、子面板的預設集、自動套用子面板預設集、自適應父面板模式，與父面板導覽預設集。
- **與父面板連動**：新子面板預設是否與父面板同步資料夾焦點，同視窗與新視窗可分別設定。
- **Folder Notes**：在純資料夾檢視中停用資料夾筆記開啟。
