...
- **單列精簡 Header**：Folder Space 的 header 只佔一列（原生檔案總管工具列已隱藏——新增 note／資料夾等檔案動作可從標題列的右鍵原生選單取得）：
  - 顯示目前根目錄路徑（截斷顯示，滑鼠懸停以 tooltip 顯示完整路徑）。
  - 點擊根目錄標題，開啟可搜尋的資料夾選擇器來切換根目錄——可用來返回上層資料夾，或直接跳到任一子資料夾。
  - 對標頭文字按右鍵，呼叫 Obsidian 原生資料夾右鍵選單 (`file-menu`)——包含新增 note／新增資料夾等完整檔案動作。
  - `檢視設定` 按鈕 (⋮)：檢視預設集、樹狀／扁平樣式、展開層級（1／2／全部）、顯示內容（資料夾／檔案／全部），以及設定資料夾圖示。
  - 重新命名根資料夾時自動彈出安全重命名對話框並同步更新工作區。
- **自訂頁籤圖示**：提供可搜尋的圖示選擇器，從 `檢視設定` 選單為每個根資料夾單獨設定自訂圖示。
