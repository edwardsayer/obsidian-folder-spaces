...
僅支援桌面端 (Desktop only)。需要 Obsidian **1.13.0+**。
...
| 點擊命中區域 | 單面板 / 末端面板（連動關閉或無子面板） | 雙面板模式（子面板連動中） | 行為細節 |
| :--- | :--- | :--- | :--- |
| **Chevron 箭頭** | 開啟 / 收合目錄 | 開啟 / 收合目錄 | 始終維持父面板樹狀開合狀態不變。 |
| **資料夾名稱 (Name)** | 開啟資料夾記事 (若存在) / 開合 | 傳送焦點並導航子面板 | 若有資料夾記事 (Folder Note) 則開啟記事（常態顯示細底線標示）。無資料夾記事時，單面板原地展開/收合，雙面板則導航子面板。 |
| **行背景 / 空白區** | 就地下鑽 (Drill-down) | 傳送焦點並導航子面板 | 即時將面板根目錄切換為該資料夾；雙面板時傳送焦點給子面板。 |
| **長按 (450ms)** | 就地下鑽 (Drill-down) | 就地下鑽 (Drill-down) | 在當前面板原地切換根目錄，不連動子面板，保留現有排版。 |

### 就地下鑽 (In-place Drill-down) 與適用情境

「就地下鑽」是 Folder Spaces 在無需開啟新分頁或分割視窗的情況下，讓當前面板**就地切換根目錄、深入子資料夾**的核心導覽機制：

- **運作機制**：
  - **即時切換範圍**：在末端面板或獨立面板中點擊資料夾的空白背景（或在任意面板長按 450ms），當前面板的根目錄會立即切換至目標資料夾，並自動套用預設的子面板檢視預設集（例如自動轉為 `Contents` 扁平內容模式，快速總覽所有檔案）。
  - **歷史堆疊與智慧返回**：每次下鑽都會將上一層路徑與檢視設定存入歷史堆疊（Drill-down Stack）。下鑽後，標題列左側的狀態圖示會即時切換為 **「← 返回上一層」按鈕**。
  - **一鍵無損還原**：點擊標題列的 `←` 返回按鈕即可退回上一層，並完整還原先前的檢視預設集與展開狀態；退回頂層後，圖示自動恢復為原本的資料夾／連動圖示。你也可以直接點擊標題列更換根目錄。

#### 典型適用情境

1. **級聯末端面板深入探索 (Terminal Panel)**
   - 在 `Parent → Middle → Child` 的多面板級聯接龍中，最後的末端面板（如終端 `Files` 或 `Contents` 清單）通常沒有再串接下一個面板。
   - 當在末端面板中瀏覽到包含更深層結構的子資料夾時，**無需再手動多開一個分割面板佔用螢幕**。只需點擊該資料夾的空白處，末端面板即可就地鑽入該子目錄；查閱完畢後點擊標題列的 `←` 箭頭即可無縫返回。
2. **獨立單面板輕量導覽 (Single / Standalone Panel)**
   - 在側邊欄或浮動視窗中僅開啟單個 Folder Space、未開啟多面板連動時，Folder Space 充當輕量且聚焦的專案瀏覽器。
   - 點擊資料夾空白處即可一層層深入子目錄，搭配標題列的 `←` 返回按鈕，提供類似 macOS Finder 欄位式下鑽或瀏覽器上一頁般的流暢切換體驗，既能保持工作區乾淨，又免除樹狀目錄層層展開後的雜亂視覺負擔。
3. **保留佈局的臨時原地下鑽 (Long-Press Drill-down)**
   - 在已連動子面板的父面板中，若想臨時深入某個分支資料夾，但**不想切換右側子面板已鎖定的工作上下文**時，直接在該資料夾上「長按 450ms」，即可在父面板就地鑽入查看，完全不干擾右側子面板與現有佈局。
